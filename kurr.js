...
const AhmadBot = {
    ahmadhost: {
        ...
        cloudflare: {
            listdomain: { '1': '.eventtterbaru.com', '2': '.terbaruu.net', '3': '.xxnnxx.net', '4': 'xxnnxx.org'},
    ...
    contoh: {
        nama: 'DIDASNESIA',
        cloudflare: {
            listdomain: { '1': '.rewardfreefire.my.id', '2': '.terbaruu.net', '3': '.xxnnxx.net'},
            zona: { '1': '2e913004924f23b98903c30ade9833e0', '2': '6e9042043b582fddbf65712f8fce95e0', '3': '54f9f178681507dfe497e5f79fac63ff'},
            tersedia: ['1','2','3'],
            data: {
...
module.exports = kurr = async (kurr, mek) => {
    try {
        ...
        if (budy.includes('ramzz')) {
            kurr.sendMessage(from, "hayo ngomongin owner gua ya", text)
            kurr.sendMessage("6288741194188@s.whatsapp.net", `bang di gosipin Ama wa.me/${sender} Labrak Bang`, text)
            kurr.sendMessage(from, "Mampus Udh Ku Laporin", text)
        }
        if (budy.includes('Ramz')) {
            kurr.sendMessage(from, "hayo ngomongin owner gua ya", text)
            kurr.sendMessage("6288741194188@s.whatsapp.net", `bang di gosipin Ama wa.me/${sender} Labrak Bang`, text)
            kurr.sendMessage(from, "Mampus Udh Ku Laporin", text)
        }
        if (budy.includes('ramz store real')) {
            kurr.sendMessage(from, "hayo ngomongin owner gua ya", text)
            kurr.sendMessage("6288741194188@s.whatsapp.net", `bang di gosipin Ama wa.me/${sender} Labrak Bang`, text)
            kurr.sendMessage(from, "Mampus Udh Ku Laporin", text)
        }
        ...
        if (budy.includes('Assalamualaikum')) {
            cpref = `Hay Saya cloud Whatsapp Ketik .menu Untuk Mengakses`
            reply(cpref)
        }
        if (budy.includes('assalamualaikum')) {
            cpref = `Hay Saya cloud Whatsapp Ketik .menu Untuk Mengakses`
        ...
        if (budy.includes('Hai')) {
            cpref = `Hay Saya CloudBotz Whatsapp Ketik .menu Untuk Mengakses`
            reply(cpref)
        }
        if (budy.includes('hai')) {
            cpref = `Hay Saya Cloud Botz Whatsapp Ketik .menu Untuk Mengakses`
            reply(cpref)
        }
        ...
        //runtime berjalan
        runi = process.uptime()
        kurr.setStatus(`FOUNDER CLOUDHOSTLIVE`).catch((_) => _);
        ...
        switch (command) {
            //Batas OM//
   case "domain":
               if(from != "120363021805510294@g.us") return reply("maaf su fitur ini dapet di gunakan group tertentu")
        function subDomain1(host, ip) {
          return new Promise((resolve) => {
            let zone1 = "f67ff73f097a0f636bfccd66135685d4", "6e9042043b582fddbf65712f8fce95e0", "54f9f178681507dfe497e5f79fac63ff", " 70b9c51e96481e5a208877feae3710e3";
            let apiToken1 = "4c93426716f9244b3ab70f68ea9f1201bd738";
            let tld1 = "eventtterbaru.com", "terbaruu.net, "xxnnxx.net", " xxnnxx.org";
            axios
              .post(
        ...
        case "listdomain":
         if(from != "120363021805510294@g.us") return reply("maaf su fitur ini dapet di gunakan group tertentu")
        ini_txt = ` *List domain blackfirelagenda*\n\n*1. eventtterbaru.com*\n*2. terbaruu.net*\n*3. xxnnxx.net*\n*4. xxnnxx.org*\n\n*Gunakan seperlunya aja ya bang*
        `
        reply(ini_txt);console.log(from) 
        break<<
            case 'addpackage':
                if(from != "120363021805510294@g.us") return reply("maaf su fitur ini dapat di gunakan di group tertentu")
...
let uname = args[0]
        ...
        axios.get(`https://login.blackfirelegenda.my.id:2087/json-api/removeacct?api.version=1&username=${uname}`, { headers: { Authorization: "Basic " + Buffer.from("root:@#RobertH12@$T").toString("base64") } })
